...
platform_do_upgrade_ap7612() {
   local tar_file="$1"

mkdir -p /tmp/sysup
tar -xf "$tar_file" -C /tmp/sysup

ubidetach -p /dev/mtd15 2>/dev/null

# Check if UBI volumes exist
if ! ubiattach -p /dev/mtd15 2>/dev/null || [ ! -e /dev/ubi0_0 ] || [ ! -e /dev/ubi0_1 ] || [ ! -e /dev/ubi0_2 ]; then
    echo "Partitions missing, creating UBI layout..."
    ubiformat /dev/mtd15 -y
    ubiattach -p /dev/mtd15

    ubimkvol /dev/ubi0 -N kernel -s 6MiB
    ubimkvol /dev/ubi0 -N rootfs -s 30MiB -t static
    ubimkvol /dev/ubi0 -N rootfs_data -m -t dynamic
fi

  ubiupdatevol /dev/ubi0_0 /tmp/sysup/sysupgrade-ap7612/kernel
  ubiupdatevol /dev/ubi0_1 /tmp/sysup/sysupgrade-ap7612/root
sync
